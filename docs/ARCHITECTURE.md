...
- 初始窗口 `128 KiB` 是 **floor**。接收端按 **有序交付环**（loop）而不是 RTT 定窗：`target = clamp(2 × deliver_rate × (rtt + hole), 128 KiB, 128 KiB × chan)`。`rtt` 是到达路径（`last_recv_path`，其次 sticky，再次池 min）的 EWMA，**上限 2 × 该路 10 s 窗 min RTT**（EWMA 会跟着窗口自己造出的队列涨，`W' = W·2R/L` 是正反馈）；`hole` 是重排缓冲里一个字节的平均驻留时间 = `hole_bytes_ewma / deliver_rate`（Little's law，字节加权；按事件加权的 per-hole EWMA 在大量短重排洞夹几个 RTO 长洞时会低读，只留作 `nya.hole_us` 观测），`8 × loop` 内没有新样本就视为 0。cap **单调上涨**（`max(cur, target)`；wire 由每路 budget 兜底，窗口大只花内存），只有应用落后（`app_backlog > cap/2`）才向 `target` 收缩、且不低于当前持有字节，`advertised_window ≥ 0` 恒成立。会话级内存护栏 `recv_cap_extra = Σ(recv_cap − floor) ≤ alive_paths × ceil` 只限制上涨。P3b 的 edge-ring / 翻倍探测已删除。不是 TOML 开关，也不改 `Tuning::STANDARD.initial_window`。
...
- 一条 bulk 流尽量钉在一条 5-tuple（`bulk_affinity`，含正在 flush 的 write-stalled dest；P7：sticky 用 quiet-fresh = `degrade_timeout` 内有收包，不是 `loss_timeout`）。交互 affinity 仍跳过 write-stalled。
- **Loop-aware 放置（P3）**：每条路的期望驻留 `sojourn = loaded ∨ decayed ∨ min_rtt`——`loaded` 是新鲜 `ack_rtt`（空闲超过 `bw_window_us` 即过期，KD7）并以「有 outstanding 字节的静默时长」为下限（卡死的路没样本也读 ≥ 静默时长；首轮无样本时下限不低于自身 RTT），`decayed` 是最后一次 loaded 值在 10 s 内线性衰减到 `min_rtt`。`maintain` 每 tick 取 alive ∧ schedulable 的最小 `sojourn` 作 `pool_ref`；一条路 **相对池** 是 backup（`2× + 20 ms`）才算 loop-unfit，从未 loaded 恒 fit，只剩慢路时最不差的也 fit。bulk 选路统一走 `scheduler::pick_bulk`：schedulable ⇒ quiet-fresh-or-all ⇒ 有 budget 余量 ⇒ fit-or-all ⇒ 优先无交互流的 TCP ⇒ score。转 bulk 时先 HOL 再 fit；HOL fallback fit-or-all；**溢出是 fit-or-wait**（unfit 兄弟不接溢出，没 fit 的就 `budget_wait`）；sticky 满的 fallback 只找 fit 的、否则 park 而 **不** `set_sticky`；没有 sticky 才在 any-room 护栏内 fit-or-all 重新安家。P3.3：sticky 的 loop 变 unfit 时**只换一次**到有余量、无交互流、fit 的兄弟（`nya_migrates_loop_unfit_total`，不算 HOL），每流 10 s 一次，每个目的路径在拿到一个 `ack_rtt` 样本且过了 `bw_window_us` 前只接一条（防 herd）。已发出的 piece 留在原路（KD6）。
- **Budget 标记（P4）**：budget 只在「这一轮把 bulk 挡回去」时涨。sticky 满而分流到兄弟（`send_budget_diverts`）时标 sticky `round_limited`；兄弟因为自己 budget 满被 pick 跳过时也标兄弟（`has_room_or_mark`）——否则承担三分之一拷贝的溢出目标会永远停在 64 KiB floor。
