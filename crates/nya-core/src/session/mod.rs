...
impl Session {
    ...
    pub fn shutdown(&self) {
            ...
            .is_ok()
        {
            ...
            let streams: Vec<_> = self
                ...
                .collect();
            for s in streams {
                let _ = s.inbound_tx.try_send(crate::stream::Inbound::Reset(
                    nya_proto::ResetReason::SessionDead,
                ));
            }
        }
    }
    ...
    pub fn snapshot(&self) -> Snapshot {
        let paths = self.path_list();
        self.inner.metrics.snap_with_paths(&paths)
    }
...
impl SessionTable {
    ...
    pub fn create_with_incoming(
        ...
        let (session, rx) = Session::new_server(self.cfg.clone());
        self.sessions
            .lock()
            .unwrap()
